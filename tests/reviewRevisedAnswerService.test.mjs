...
import {
  ReviewRevisedAnswerServiceError,
  createReviewRevisedAnswerService,
} from "../src/lib/v3/reviewRevisedAnswerService.ts";
import {
...
const ENDPOINT = "/.netlify/functions/review-revised-answer";
const REQUEST = {
  contractVersion: "1",
...
const SUCCESS_META = {
  traceId: "trace-review-service-success",
  modelLatencyMs: 140,
  usage: {
    inputTokens: 480,
    outputTokens: 160,
...
function createError(failure = {
  ...
  return {
    ...
    ok: false,
    error: failure,
    meta: { traceId: "trace-review-service-error" },
  };
}
...
async function captureServiceError(promise) {
  ...
  await assert.rejects(promise, (error) => {
    assert.ok(error instanceof ReviewRevisedAnswerServiceError);
    captured = error;
    return true;
  });
...
test("posts the exact shared request once to the fixed same-origin endpoint", async () => {
  const envelope = createSuccess(validResolvedRevisionComparison);
  const harness = createFetchHarness({ body: envelope });
  const service = createReviewRevisedAnswerService(
  ...
  const sentRequest = JSON.parse(harness.calls[0].init.body);
  assert.deepEqual(sentRequest, REQUEST);
  assert.equal(sentRequest.originalAnswer, flawedStateOwnershipAnswer);
  assert.equal(sentRequest.revisedAnswer, revisedStateOwnershipAnswer);
  assert.deepEqual(sentRequest.diagnosis, validNeedsFollowUpDiagnosis);
  for (const serverOwnedOrCorrelationField of [
    ...
    assert.equal(
      Object.hasOwn(sentRequest, serverOwnedOrCorrelationField),
      false,
    );
  }
...
test("accepts valid success envelopes with or without a next action", async () => {
  const results = [
    validResolvedRevisionComparison,
    {
      ...validResolvedRevisionComparison,
  ...
  for (const result of results) {
    ...
    const harness = createFetchHarness({ body: envelope });
    const response = await createReviewRevisedAnswerService(
      harness.fetchImplementation,
    )(REQUEST);
...
test("rejects unreadable JSON and malformed envelopes safely", async () => {
  const invalidBodies = [
    ...
    {
      ...createSuccess(validResolvedRevisionComparison),
  ...
  for (const body of invalidBodies) {
    ...
    const error = await captureServiceError(
      createReviewRevisedAnswerService(harness.fetchImplementation)(REQUEST),
    );

    assert.equal(harness.calls.length, 1);
    assert.equal(harness.jsonReadCount, 1);
    assert.equal(error.message.includes(flawedStateOwnershipAnswer), false);
    assert.equal(error.message.includes(revisedStateOwnershipAnswer), false);
...
test("rejects HTTP and envelope outcome mismatches", async () => {
  const mismatches = [
    ...
    { body: createSuccess(validResolvedRevisionComparison), ok: false },
  ];
